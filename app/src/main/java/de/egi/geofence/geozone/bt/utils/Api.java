...
import de.egi.geofence.geozone.bt.ssl.SSLContextFactory;
...
public class Api {
    ...
    public String doGet(boolean isCsrf)  throws Exception {
        String result;
        URL requestedUrl;
        HttpURLConnection urlConnection = null;
        try {
            // For Fhem to get the csrf token
            if (authParams.getUrlXHR() != null && !authParams.getUrlXHR().equals("") && isCsrf){
                requestedUrl = new URL(authParams.getUrlXHR());
            }else {
                requestedUrl = new URL(authParams.getUrl());
            }
...
//                ((HttpsURLConnection)urlConnection).setHostnameVerifier(org.apache.http.conn.ssl.SSLSocketFactory.ALLOW_ALL_HOSTNAME_VERIFIER);
            ...
            if (lastResponseCode >= 400){
                ...
                if (authParams.getUrlXHR() != null && !authParams.getUrlXHR().equals("") && isCsrf) {
                    // Get the csrf token
                    result = urlConnection.getHeaderField("X-FHEM-csrfToken");
                }else {
