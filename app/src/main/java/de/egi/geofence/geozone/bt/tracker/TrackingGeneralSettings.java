...
import de.egi.geofence.geozone.bt.utils.SharedPrefsUtil;
import de.egi.geofence.geozone.bt.geofence.SimpleGeofenceStore;
import de.egi.geofence.geozone.bt.utils.Utils;

public class TrackingGeneralSettings extends AppCompatActivity implements OnClickListener, TextWatcher, OnItemSelectedListener, CompoundButton.OnCheckedChangeListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(de.egi.geofence.geozone.bt.R.layout.general_tracking_settings);
		sharedPrefsUtil = new SharedPrefsUtil(this);
        Spinner spinnerLocPriority = (Spinner) findViewById(de.egi.geofence.geozone.bt.R.id.spinner_loc_prio);
		...
		enter = (SwitchCompat) this.findViewById(R.id.stopTrackingAtEnter);
		exit = (SwitchCompat) this.findViewById(R.id.stopTrackingAtExit);
		enter.setOnCheckedChangeListener(this);
		exit.setOnCheckedChangeListener(this);
		((SwitchCompat) this.findViewById(R.id.stopTrackingAtEnter)).setChecked(Utils.isBoolean(dbGlobalsHelper.getCursorGlobalsByKey(Constants.DB_KEY_STOPTRACKINGATENTER)));
		((SwitchCompat) this.findViewById(R.id.stopTrackingAtExit)).setChecked(Utils.isBoolean(dbGlobalsHelper.getCursorGlobalsByKey(Constants.DB_KEY_STOPTRACKINGATEXIT)));

		locInterval = ((EditText) this.findViewById(de.egi.geofence.geozone.bt.R.id.value_loc_interval));
		...
		ampel = ((ImageView) this.findViewById(de.egi.geofence.geozone.bt.R.id.tracking_service_state));

		if (TrackingUtils.isMyServiceRunning(TrackingLocationService.class, this)){
			ampel.setImageResource(de.egi.geofence.geozone.bt.R.drawable.ic_lens_green_24dp);
		}else{
			ampel.setImageResource(de.egi.geofence.geozone.bt.R.drawable.ic_lens_red_24dp);
		}
        ...
        Button stop = ((Button) this.findViewById(de.egi.geofence.geozone.bt.R.id.bt_stop_tracking));
		stop.setOnClickListener(this);
		
		// Location priority spinner
		List<String> locPrio_li = new ArrayList<>();
		locPrio_li.add(this.getString(de.egi.geofence.geozone.bt.R.string.prio_low));
		locPrio_li.add(this.getString(de.egi.geofence.geozone.bt.R.string.prio_balanced));
		locPrio_li.add(this.getString(de.egi.geofence.geozone.bt.R.string.prio_high));

		ArrayAdapter<String> adapter_loc_prio = new ArrayAdapter<>(this, de.egi.geofence.geozone.bt.R.layout.spinner_item, locPrio_li);
		adapter_loc_prio.setDropDownViewResource(de.egi.geofence.geozone.bt.R.layout.spinner_dropdown_item);
	...
	public void onClick(View v) {
		...
		case de.egi.geofence.geozone.bt.R.id.tracking_service_state:
	...
	public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		// Location priority
		if (arg0.getId() == de.egi.geofence.geozone.bt.R.id.spinner_loc_prio){
	        String prio = arg0.getSelectedItem().toString();
        	if (prio.equalsIgnoreCase(this.getString(de.egi.geofence.geozone.bt.R.string.prio_low))){
				dbGlobalsHelper.storeGlobals(Constants.DB_KEY_LOCPRIORITY, Integer.valueOf(LocationRequest.PRIORITY_LOW_POWER).toString());
        	}
        	if (prio.equalsIgnoreCase(this.getString(de.egi.geofence.geozone.bt.R.string.prio_balanced))){
        	...
        	if (prio.equalsIgnoreCase(this.getString(de.egi.geofence.geozone.bt.R.string.prio_high))){
	...
	public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
		...
		if (buttonView == enter) {
			if (isChecked) {
				dbGlobalsHelper.storeGlobals(Constants.DB_KEY_STOPTRACKINGATENTER, "true");
...
//				}
				...
				dbGlobalsHelper.storeGlobals(Constants.DB_KEY_STOPTRACKINGATENTER, "false");
//				// Stop tracking
...
//					TrackingUtils.stopTracking(this, ze.getName());
		...
		if (buttonView == exit) {
			if (isChecked) {
				dbGlobalsHelper.storeGlobals(Constants.DB_KEY_STOPTRACKINGATEXIT, "true");
...
//				}
				...
				dbGlobalsHelper.storeGlobals(Constants.DB_KEY_STOPTRACKINGATEXIT, "false");
//				// Stop tracking
//				if (TrackingUtils.exists(this, ze.getName()) > 0)
//					TrackingUtils.stopTracking(this, ze.getName());
			}
