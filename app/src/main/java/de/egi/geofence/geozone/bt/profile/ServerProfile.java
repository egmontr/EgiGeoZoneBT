...
import de.egi.geofence.geozone.bt.tracker.TrackingReceiverWorkerService;
	...
	private void doTest(Location currentLocation){
		...
		try {
			worker.doServerRequest(1, this.getApplicationContext(), urlEnteredReplace, urlExitedReplace, fhem_geofancy,
					...
					(Double.valueOf(currentLocation.getLatitude()).toString()), (Double.valueOf(currentLocation.getLongitude()).toString()), true, 0);
		} catch (Exception ex) {
